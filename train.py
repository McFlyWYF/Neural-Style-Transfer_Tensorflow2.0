...
# image1 = Image.open(os.path.join(settings.CONTENT_IMAGE_PATH))
# plt.figure()
# plt.imshow(image1)
# plt.axis('off')
# plt.title('噪声图片')
...
for epoch in range(settings.EPOCHS):
    ...
    with tqdm(total=settings.STEPS_PER_EPOCH, desc='Epoch {}/{}'.format(epoch + 1, settings.EPOCHS)) as pbar:
        ...
        for step in range(settings.STEPS_PER_EPOCH):
            ...
            loss_sum.append(int(_loss))

            pbar.set_postfix({'loss': '%.4f' % float(_loss)})
            # print('Epoch : %d' % epoch,'----------step : %d'% step,'-----------loss : %.4f' % float(_loss))
            pbar.update(1)
        # 每个epoch保存一次图片
    ...
    lena1 = Image.open(os.path.join('./output/' + str(epoch + 1) + '.jpg'))
    # plt.subplot(1, 2, 1)
    plt.imshow(lena1)
    plt.axis('on')
    plt.title('风格转换图')
    ...
    output = open('./loss/loss_' + str(epoch + 1) + '.xls','w',encoding='gbk')

    for i in range(len(loss_sum)):
        output.write(str(loss_sum[i]))
        output.write('\n')
    output.close()